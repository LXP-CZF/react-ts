                    ...
                    <div className="tabs-order">
                        <ul className="sort-container">
                            {this.state.tabs.map(tab => (
                                <li key={tab.id} className={tab.type === this.state.active ? 'on' : ''} onClick={() => this.changeTab(tab.type)}>
                                    按{tab.name}排序
                                </li>
                            ))}
                        </ul>
                    ...
                    <div className="comment-send">
                        ...
                        <div className="textarea-container">
                            ...
                            <textarea
                                ...
                                placeholder="发条友善的评论"
                                className="ipt-txt" />
                            <button className="comment-submit" onClick={this.handleAddComment}>发表评论</button>
                        </div>
                                ...
                                <div className="comment">
                                    ...
                                    <div className="info">
                                        ...
                                        <span className={item.attitude === 1 ? 'like liked' : 'like'}>
                                            <i className="icon" onClick={() => this.handleAttitude(item.id, 1)}/>
                                        </span>
                                        <span className={item.attitude === -1 ? 'hate hated' : 'hate'}>
                                            <i className="icon" onClick={() => this.handleAttitude(item.id, -1)} />
                                        </span>
                                        <span className="reply btn-hover" onClick={() => this.handleDelComment(item.id)}>删除</span>
                                    </div>
                                </div>
                            </div>
                        ))}
