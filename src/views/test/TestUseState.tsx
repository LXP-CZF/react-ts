...
const TestUseState: React.FC<propsType> = (props) => {
  const [params] = useSearchParams() // /use?id=001
  const id = params.get('id')
  const param = useParams() // /use/002
  ...
  return (
    <customContext.Provider
      value={'通过createContext.Provider传递数据，利用useContext函数获取数据'}
    >
      <span>
        路由参数{id}
        {param.id}
      ...
      <SonA
        msg={username}
        changeMsg={function (newMsg: string): void {
